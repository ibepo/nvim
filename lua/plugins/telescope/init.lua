...
require("telescope").setup({
	extensions = {
		...
		command_palette = {
			...
			{ "Terminal", { "Vertical Right", ":vsp | terminal", 1 } },
			{
				"Notes",
...
function M.grep_nvim_src()
	require("telescope.builtin").grep_string({
		...
		path_display = { "smart" },
		search_dirs = { "~/vim-dev/sources/neovim/runtime/lua/vim/", "~/vim-dev/sources/neovim/src/nvim/" },
	})
end
...
function M.find_configs()
	require("telescope.builtin").find_files({
		...
		path_display = { "smart" },
		search_dirs = { "~/.oh-my-zsh/custom", "~/.config/nvim", "~/.config/alacritty" },
		-- cwd = "~/.config/nvim/",
		layout_strategy = "horizontal",
		layout_config = {
			preview_width = 0.65,
